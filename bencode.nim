...
type
  ...
  BencodeValue* = ref object
    ...
    of bk_dict: dict_val*: Ordered_table[string, BencodeValue]

  BencodeError* = object of Catchable_error

proc new_int*(val: int): BencodeValue =
  BencodeValue(kind: bk_int, int_val: val)

proc new_string*(val: string): BencodeValue =
  BencodeValue(kind: bk_string, str_val: val)

proc new_list*(val: seq[BencodeValue] = @[]): BencodeValue =
  BencodeValue(kind: bk_list, list_val: val)

proc new_dict*(val: Ordered_table[string, BencodeValue] = init_ordered_table[string, BencodeValue]()): BencodeValue =
  BencodeValue(kind: bk_dict, dict_val: val)
...
proc parse_int(data: string, pos: var int): BencodeValue =
  if data[pos] != 'i':
    raise new_exception(BencodeError, "Expected 'i' at start of integer")
  ...
  while pos < data.len and data[pos] != 'e':
    if data[pos] notin Digits and data[pos] != '-':
      raise new_exception(BencodeError, "Invalid character in integer")
    inc pos
  
  if pos >= data.len:
    raise new_exception(BencodeError, "Unterminated integer")
  ...
  try:
    ...
    raise new_exception(BencodeError, "Invalid integer: " & int_str)
...
proc parse_string(data: string, pos: var int): BencodeValue =
  ...
  if pos >= data.len or data[pos] != ':':
    raise new_exception(BencodeError, "Expected ':' after string length")
  ...
  try:
    ...
    if pos + length > data.len:
      raise new_exception(BencodeError, "String length exceeds data")
    ...
    raise new_exception(BencodeError, "Invalid string length: " & length_str)

proc parse_list(data: string, pos: var int): BencodeValue =
  if data[pos] != 'l':
    raise new_exception(BencodeError, "Expected 'l' at start of list")
  ...
  if pos >= data.len:
    raise new_exception(BencodeError, "Unterminated list")
  
  inc pos  # Skip 'e'

proc parse_dict(data: string, pos: var int): BencodeValue =
  if data[pos] != 'd':
    raise new_exception(BencodeError, "Expected 'd' at start of dictionary")
  ...
  while pos < data.len and data[pos] != 'e':
    ...
    if key.kind != bk_string:
      raise new_exception(BencodeError, "Dictionary key must be string")
    
    let value = parse_bencode(data, pos)
    result.dict_val[key.str_val] = value
  
  if pos >= data.len:
    raise new_exception(BencodeError, "Unterminated dictionary")
  
  inc pos  # Skip 'e'

proc parse_bencode*(data: string, pos: var int): BencodeValue =
  if pos >= data.len:
    raise new_exception(BencodeError, "Unexpected end of data")
  ...
  of '0'..'9': result = parse_string(data, pos)
    ...
    raise new_exception(BencodeError, "Invalid bencode character: " & $data[pos])
